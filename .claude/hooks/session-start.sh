...
echo "  4. decisions    — mark inline: **Decision (topic) — category: status**"
echo "  5. session-end  — /task-manager + /decision-journal harvests both fire"

# --- Build workflow (most common session type) ---
echo ""
echo "BUILD WORKFLOW — follow this order when implementing a task:"
echo "  Step 1  /task-manager           surface task + acceptance criteria"
echo "  Step 2  /algo-trading-lead-dev  plan (ends: 'Plan ready.' or 'Blocked')"
echo "  Step 3  /algo-trading-lead-dev  build — write failing test FIRST"
echo "  Step 4  /go-quality-review      standard level [REQUIRED if internal/ or pkg/ touched]"
echo "  Step 5  /task-manager           verify criteria + mark done"
echo "  Step 6  → session-end           /task-manager harvest + /decision-journal harvest"
echo ""
echo "Other workflows: review.md · evaluate.md · bugfix.md · code-review.md"
echo "Consult workflows/INDEX.md if the session doesn't match the build workflow."
